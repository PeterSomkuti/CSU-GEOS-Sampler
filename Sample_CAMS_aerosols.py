...
def main(scene_h5, aerdb, modelpath):
    ...
    aernames = {
        'aermr01': "/SS1", 'aermr02': "/SS2", 'aermr03': "/SS3",
        'aermr04': "/DD1", 'aermr05': "/DD2", 'aermr06': "/DD3",
        'aermr07': "/OM_phil", 'aermr08': "/OM_phob",
        'aermr09': "/BC_phil", 'aermr10': "/BC_phob",
        'aermr11': "/SU",
    }
    ...
    scene_times = num2date(
        scene_tai, units='Seconds since 1993-01-01',
    ...
    date_list = pd.date_range(ymd_min.strftime("%Y-%m-%d"), ymd_max.strftime("%Y-%m-%d"), freq='D')
    ...
    for i, fname in enumerate(filenames):
        with Dataset(fname, 'r') as nc:
            ...
            _tmp_time = num2date(
                nc.variables[time_var][:], units=nc.variables[time_var].units,
                only_use_cftime_datetimes=False, only_use_python_datetimes=True
            )
    ...
    scene_plevels = scene_h5['Simulation/Thermodynamic/pressure_level'][:, :scene_nlayers + 1]
    scene_tlevels = scene_h5['Simulation/Thermodynamic/temperature_level'][:, :scene_nlayers + 1]
    scene_glayers = scene_h5['Simulation/Thermodynamic/gravity_layer'][:, :scene_nlayers]

    scene_plevels[scene_plevels == 0.] = 1.0
    ...
    sample_coords = np.array([
        ...
        for i in range(n_scene) for j in range(scene_nlayers + 1)
    ])

    logger.info("Sampling specific humidity and aerosols...")
    grid_axes = (cams_dict['tai'], cams_dict['logp'], cams_dict['latitude'], cams_dict['longitude'])

    # Process Specific Humidity
    rgi_q = spi.RegularGridInterpolator(grid_axes, cams_dict['q'], bounds_error=False, fill_value=None)
    scene_sampled_q = rgi_q(sample_coords).reshape(n_scene, scene_nlayers + 1)

    scene_sampled_mmr = {}
    for aer in tqdm(aernames.keys(), desc="Sampling Aerosols"):
        rgi = spi.RegularGridInterpolator(grid_axes, cams_dict[aer], bounds_error=False, fill_value=None)
    ...
    for i in range(n_scene):
        idx_noaer = [
            j for j in range(species_density.shape[1])
    ...
    for grp, aers in aergroups.items():
        for aer in aers:
            mmr_layer = 0.5 * (scene_sampled_mmr[aer][:, :-1] + scene_sampled_mmr[aer][:, 1:])
    ...
    if args.scale:
        for aer in scene_sampled_mmr.keys():
            ...
            relative_group_weight = scene_sampled_aer_mass[aer] / scene_sampled_grp_mass[grp]
            ...
            scene_sampled_mmr[aer] *= scale_factor[:, np.newaxis] * relative_group_weight[:, np.newaxis]
    else:
        logging.info("Not scaling aerosol masses!")

    rhs = aerdb.root.rh.read()
    if np.count_nonzero(rhs == 0.) != 1:
    ...
    for aer in scene_sampled_mmr.keys():
        ...
        for i in tqdm(range(n_scene), desc=f"Mapping {aer} to slots", leave=False):
            ...
            un_rhs = np.unique(scene_layerrh[i][particle_counts[aer][i] > 0.]) if rh_dep else [0]
            none_idx = np.where((species_id[i] == b'none') | (species_id[i] == b''))[0]

            if len(none_idx) < len(un_rhs):
                logger.critical(f"ERROR! No more space for scene #{i}. Extend slots or use fewer species.")
                sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scene", help="Path to single scene file.", type=str, required=True)
    parser.add_argument("--aerdb", help="Path to CAMS aerosol database.", type=str, required=True)
    parser.add_argument("--modelpath", help="Path to CAMS model profiles (top-level)", type=str, required=True)
